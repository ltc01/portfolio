            ...
            <div className='flex items-center justify-between'>
                <div className="mx-10">
                    <h4 className="uppercase font-bold text-4xl">
                        Lax<span className="text-blue-500">mi</span>
                    </h4>
                </div>
                <div className={`${sticky? "md: bg-white/0 bg-white": "bg-white" } text-gray-900 md:block hidden px-7 py-2 font-medium rounded-bl-full`}>
                    <ul className="flex items-center gap-1 py-2 text-lg">
                        {menuLinks?.map((menu,i)=> (
                            <li key={i} className="hover:text-blue-400 px-10">
                ...
                className={`z-[999] ${open? "text-gray-900" :"text-gray-100"} text-4xl md:hidden m-5`}>
                    <ion-icon name="menu"></ion-icon>
                </div>
                <div className={`md:hidden text-gray-900 absolute w-2/3 h-screen px-7 py-2 font-medium duration-300 bg-white top-0 ${ open ? "right-0" : "right-[-100%]"}`}>
